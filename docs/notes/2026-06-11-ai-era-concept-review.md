...
> ⚠️ **次セッションへの引き継ぎ（これを読んでから本文へ）**
> 本ノートは議論ログであり、**途中の結論は後で覆っているものがある**
...
>    → **完了（2026-06-12）**: change `remove-hook-notification` で撤去済み
> 4. Windows 実機での OSC 送信側確認（リリース前・要 Windows 機）
...
## ステータス（このテーマの第 1 弾は完了）

- 2026-06-11: **PR #103 マージ・実装完了**。OSC タスク通知（ADR-0066）が main に合流。
  macOS 実機確認済み（許可待ち即時・入力待ち 60 秒・クリック復帰）。
  change `osc-task-notification` はアーカイブ済み、Issue #85 クローズ。
  残: Windows 実機確認（リリース前）・フック経路（ADR-0057）撤去の後続 change。
  次のテーマ候補は本ノート前半の優先順位どおり（git worktree 統合・OSC 133 状態検知）。
- 2026-06-12: **フック経路（ADR-0057）撤去完了**。OSC 通知の安定をオーナーが確認し、
...
- 2026-06-11: **再改訂（最終）** — バイナリ解析と Web 調査で、**Ghostty 等の既存ターミナルもネイティブ通知は同じ意味論**（許可待ち即時・完了相当は 60 秒アイドル。即時完了に見える事例はフック併用か閾値カスタム）と確認。「他に無い即時完了通知のためにフックを残す」根拠が消えたため、オーナー判断で**フック経路は当初方針どおり安定確認後に撤去**へ戻した。両経路を抑止し合わせない実装（D5 撤廃後のコード）は撤去前提でも正しいため、コード変更なし・ドキュメントのみ再改訂。許可待ち即時通知の検証用に `tools/test-skills` へ `notify-permission-test` スキルを追加。
- 2026-06-12: **フック経路の撤去を実施**（change `remove-hook-notification`）。ADR-0066 Decision 7 の実行。設定の永続化（enabled / preferredPort）も使途消滅のため同時撤去し、無効化は OS の通知設定に委ねる。自動インストール済みユーザーの `~/.claude/settings.json` には触れず、README で手動削除を案内。
