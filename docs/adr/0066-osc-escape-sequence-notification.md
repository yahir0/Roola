...
7. ADR-0057 の実装（HTTP 受信口・トークン・フックインストーラ・設定画面のフック節）は
   **「即時のタスク完了通知」が欲しいユーザー向けのオプション機能として存続させる**。
   claude のネイティブ通知は許可待ち（即時）と入力待ちアイドル
   （`messageIdleNotifThresholdMs` 既定 60 秒）のみで、「完了の瞬間」の通知は
   存在しないため、即時完了通知はフック経路でしか提供できない。
   両経路は通知するイベントが異なるため互いに抑止せず独立して発射する。
   （当初案の「OSC 版安定後に撤去」は実機確認後のオーナー判断で撤回・2026-06-11）
...
- **「完了の瞬間」の通知は既定では出ない**: claude ネイティブの通知意味論は
  「許可待ち（即時）」と「入力待ち 60 秒アイドル」であり、Stop フックの
  「完了した瞬間」とは異なる。既定（設定ゼロ）では完了から約 60 秒後の
  「Claude is waiting for your input」が完了通知の代替になる。即時が必要な
  ユーザーにはフック経路（Decision 7）を案内する。
