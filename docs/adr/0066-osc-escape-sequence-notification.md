...
7. ADR-0057 の実装（HTTP 受信口・トークン・フックインストーラ・設定画面のフック節）は
   ...
   - 検討の経緯: 「完了の瞬間」の通知は claude ネイティブに存在しないため、
     ...
     **Ghostty 等の既存ターミナルも同じ意味論（完了通知 = 60 秒アイドル）で
     ...
     撤去方針に戻した（2026-06-11）。
   - **撤去完了（2026-06-12）**: OSC 通知の安定をオーナー実機運用で確認し、
     change `remove-hook-notification` で ADR-0057 実装一式（HTTP 受信口・
     トークン・フックインストーラ・設定の永続化・設定画面のフック節）を撤去した。
     設定画面には通知許可状態の表示と導線のみ残る。通知発射基盤
     （`TaskNotificationRepository` / ネイティブ通知 / クリック復帰）は
     OSC 経路の共通基盤として存続。自動インストール済みユーザーの
     `~/.claude/settings.json` には触れない（残置フックは無害・手動削除を案内）。
