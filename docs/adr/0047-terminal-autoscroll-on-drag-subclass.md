...
selection"）として 2023-06 にメンテナ自身が報告しているが、約 3 年間 OPEN
のまま放置されている。Roola としては上流の修正を待たず、本 ADR の方針で
先に直す。
...
- **すぐ直る**: Roola のリポジトリ内で完結する。上流の [SwiftTerm#309]
  はメンテナ自身が 2023-06 に立てたまま 3 年弱 OPEN のため、待っていても
  体験は改善しない
...
- [SwiftTerm#309] が 3 年弱 OPEN のままなので、PR を出してもマージ
  までの時間軸が読めない（メンテナの方針 / 優先度が不明）
- 上流 issue は既に [SwiftTerm#309] に存在するため新規 issue 化は不要。
  本 ADR の Roola 側 patch は **そのうえで** 残しても良し、上流が直ったら
  撤去しても良し
...
- [Issue #56]（Roola）— ターミナルでドラッグ選択中にビューの外へ出ても
  自動スクロールしない
- [SwiftTerm#309]（上流・メンテナ自身が 2023-06 に提起した同等の issue。
  約 3 年 OPEN のまま放置）
