...
### Requirement: フック通知（ADR-0057）との共存

フック → HTTP 経路（ADR-0057）と OSC 経路は通知するイベントが異なる
（フック = タスク完了の瞬間 / OSC = 許可待ち・入力待ち 60 秒アイドル）。
両経路は互いに抑止せず独立して発射しなければならない（SHALL）。
フック経路は「即時の完了通知」が欲しいユーザー向けのオプション機能として
存続する（既定は OSC 経路のみ = 設定ゼロ）。

#### Scenario: 既定（フック未登録）では完了の瞬間の通知は出ない

- **WHEN** フック未登録の環境で claude のタスクが完了し、ユーザーが
  60 秒以上放置する
- **THEN** 完了の瞬間には通知が出ず、約 60 秒後に
  「Claude is waiting for your input」の通知が表示される

#### Scenario: フック登録済みでは完了の瞬間にも通知が出る

- **WHEN** ADR-0057 の Stop フックを登録し設定で有効化した環境で、
  claude のタスクが完了する
- **THEN** 完了の瞬間に「Claude Code が完了しました」通知が表示され、
  さらに 60 秒放置すれば OSC 経路の入力待ち通知も別途表示される
