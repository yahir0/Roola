...
同一イベント（タスク完了）が Stop フック経由と OSC 経由の両方で届きうる。
Dart の通知判断層（D2）で、OSC 経路の通知受信が機能しているペインについては
HTTP 経路の通知を破棄する（単純な時間窓のデデュープではなく経路単位で切る）。

- 代替案: 時間窓デデュープ → タイミング依存で不安定なため不採用
- フック未登録ユーザー（大多数想定）はそもそも HTTP 経路が動いていないので影響なし
