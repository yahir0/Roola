...
### D5: フック経路は「即時完了通知」のオプションとして共存させる（抑止しない）

（実装後にオーナー判断で改訂。当初は「OSC 受信実績のあるセッションのフック
通知を破棄」する重複抑止を入れたが、撤回した。）

両経路は通知するイベントが異なることが実機確認で判明した:
...
つまり 2 経路は同一イベントの重複ではなく別イベントであり、抑止し合うと
「即時の完了通知」という旧 ADR-0057 の価値が消える。決定:

- 既定（設定ゼロ）は OSC 経路のみ = 完了の瞬間は通知されず、60 秒アイドルで通知
- 即時の完了通知が欲しいユーザーは、従来どおりフックを登録して有効化する
  （オプション機能として正式に存続。**ADR-0066 当初の「安定後に撤去」は撤回**）
- 両経路は互いに独立して発射する（フック=完了時、OSC=60 秒後で時間も離れている）
