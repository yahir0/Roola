...
class AppTheme {
  ...
  static const PolarisTokens tokens = PolarisTokens(
    ...
    // machine は bg と well の中間に少しリフトしたウォームグラファイト。
    // 実機材の操作面（コンソール / ステアリングコラム）はシャシ表面に近い
    // 高さで、計器類だけが沈み込む。深さでなく「リフト＋色温度」で差別化し、
    // 純黒の空洞に落とさない（D3 の純黒回避原則）。
    machine: Color(0xFF0E0D0B), // 機材本体（ターミナル）。ウォームグラファイト
...
class PolarisTokens extends ThemeExtension<PolarisTokens> {
  ...
  /// 機材本体の地（ターミナル）。`bg` と `well` の中間に少しリフトした
  /// ウォームグラファイト。計器（well・クール・沈み込み）を「読む」のでは
  /// なく機械を「叩く」操作面に使う。リフト + 色温度で差別化することで純黒
  /// の空洞に落とさず、広い暗面が出す「void の圧」も抑える（ADR-0038 D3 改訂）。
  final Color machine;
