...
- ADR-0067: git worktree をファイラーの語彙で統合する — タイル右クリックから worktree を作成（新規 / 既存 / リモート追跡ブランチ）して隣ペインでシェル / Claude を起動。置き場所は兄弟ディレクトリ `../<repo>.worktrees/` 固定。Git ビューに一覧・状態・掃除（マージ済みワンクリック / prune / repair）、タイルにブランチ名バッジ（FS 判定・git プロセス不使用）
- ADR-0068: ワークスペースを 4 分割（2×2）まで広げる（ADR-0026 代替案 3 の射程を調整）— 既定は 3 分割のまま、タブ右クリック「右下ペインへ移動」で 4 分割になる。崩し再フローは「上段 row × 下段 row」モデルへ一般化。自動起動先は左下固定
