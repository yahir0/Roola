...
- **Task completion notifications** — get a system notification when a Claude Code
  session launched from Roola finishes a turn, via a Stop hook you add to your Claude
  settings (ADR-0057). macOS uses Notification Center; Windows uses toast notifications.
...
- **Optional Claude Code integration** — when [Claude Code](https://docs.claude.com/claude-code)
  ...
  `claude --version` healthcheck banner. Claude Code Skill launch is macOS-only.
...
- Claude Code Skill 起動は **macOS 専用**（Windows では非対応）
...
- **Windows 固有**: Claude Code Skill 起動・Claude Code 統合 UI は非対応。ターミナルは xterm.js（WebView2）ベース
