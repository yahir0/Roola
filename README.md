...
<p align="center">
  ...
  目的地へ、一瞬で。ファイル・ターミナル・Claude Code を 1 つに束ねる、開発者のためのファイルエクスプローラ。
</p>

<p align="center">
  <a href="./LICENSE"><img src="https://img.shields.io/badge/License-MIT-yellow.svg" alt="License: MIT" /></a>
  <img src="https://img.shields.io/badge/platform-macOS%2014%2B%20%7C%20Windows%2010%2F11-lightgrey" alt="Platform: macOS 14+ | Windows 10/11" />
...
**Roola** is a file explorer built for developers on **macOS and Windows**. Browse your
filesystem and reach your destinations instantly — open a shell, run an arbitrary command,
or launch a **Claude Code** skill with one click, all scoped to a directory and running
inside a built-in PTY terminal. Register frequently-used **"directory + action"** pairs
once, then launch them from the sidebar.
...
- **Built-in PTY terminal** — SwiftTerm on macOS; xterm.js (WebView2) on Windows.
  `Shift+Enter` inputs a literal newline for multi-line prompts.
...
- **Task notifications, zero-config** — Roola panes relay terminal notification
  escape sequences (OSC 9/777) to system notifications (ADR-0066). No hooks, no
  setup: Claude Code notifies instantly when it needs permission, and ~60s after
  finishing a turn while waiting for input. Any CLI tool can notify with a single
  `printf`. Click a notification to jump back to its pane. macOS uses Notification
  Center; Windows uses toast notifications.
...
- **Optional Claude Code integration** — when [Claude Code](https://docs.claude.com/claude-code)
  ...
  `claude --version` healthcheck banner.
  Roola is **not** an official Claude / Anthropic product.

### Install

**macOS**: Download the latest signed & notarized DMG from the
[Releases](https://github.com/yahir0/Roola/releases) page, drag `Roola.app` into
`Applications`, and launch from Launchpad or Spotlight. Auto-update via
[Sparkle](https://sparkle-project.org/). Requires **macOS 14 (Sonoma) or later**
(Apple Silicon & Intel).

**Windows**: Download `RoolaSetup-x.x.x.exe` from the
[Releases](https://github.com/yahir0/Roola/releases) page and run the installer.
Auto-update via [WinSparkle](https://winsparkle.org/). Requires **Windows 10/11
(64-bit)**.
...
1. **インストーラをダウンロード**: [Releases](https://github.com/yahir0/Roola/releases) ページから `RoolaSetup-x.x.x.exe` を取得。
2. **インストール**: インストーラを実行してウィザードに従う。
3. **起動**: スタートメニューまたはデスクトップショートカットから起動。

> 自分でビルドしたい場合は [Build from source](#build-from-source) を参照。

## 主な機能

- **タブ式ワークスペース** — Explorer / Terminal / Git の 3 タイプのタブを 3 ペインに自由配置（ADR-0026）。タブを右下ペインへ移すと 2×2 の 4 分割になり、Claude セッションを 2 面同時に並べられる（ADR-0068）。タブ単位で per-tab 状態を保持。
...
- **git worktree 統合** — フォルダ右クリックから worktree を切って、その場でシェル / Claude を起動（ADR-0067）。Git ビューで一覧・状態（dirty / ahead/behind / マージ済み）を確認し、マージ済みはワンクリックで掃除。worktree フォルダのタイルにはブランチ名バッジを表示。
...
- **タスク通知（設定ゼロ）** — ペイン内の CLI ツールが出す通知エスケープシーケンス（OSC 9/777）を OS 通知に中継（ADR-0066）。フック登録も設定も不要で、Claude Code の許可待ちは即時、ターン完了後の入力待ちは約 60 秒で通知。通知クリックで該当ペインへフォーカス復帰。

## 動作環境

| プラットフォーム | 要件 |
|---|---|
| **macOS** | macOS 14（Sonoma）以降（Apple Silicon / Intel） |
...
flutter run -d windows --dart-define-from-file=dart_defines/prod.json
```

`make` を使わない macOS 向け同等コマンド:
...
アプリを起動するとワークスペース（Explorer タブ）が開きます。左側にお気に入り・ランチャーのサイドバー、中央にタブ式の 3 ペイン（操作で 4 ペインに増やせます）、トップバーにアクティビティモニタ・ノートパッド・設定があります。
...
### タブ式ワークスペース（3 ペイン / 最大 4 ペイン）

タブを 3 ペイン（左上 / 右上 / 左下）に自由に配置します（ADR-0026）。タブをドラッグして移動・並べ替え、ペインの分割は境界をドラッグでリサイズ。

**4 分割にする**: タブを右クリック →「タブを右下ペインへ移動」（`⌘⌃4` / Windows `Ctrl+Alt+4`）で右下ペインが現れ、下段が左右 2 分割になります（ADR-0068）。上段と下段の左右分割位置は独立して動きます。右下ペインのタブをすべて閉じるか他ペインへ移すと、自動的に 3 分割へ戻ります。既定では右下ペインは空なので、起動直後は常に 3 分割です。
...
並列でエージェント（や自分）を働かせるときの標準作法 — 「作業 1 件 = ブランチ 1 本 = フォルダ 1 個」— をファイラーの操作で扱えます（ADR-0067）:

- **切る**: git リポジトリ配下のフォルダを右クリック →「Worktree を切って開く…」。新規ブランチ / 既存ブランチ / リモートブランチ（トラッキングして展開）を選べ、作成後にそのままシェルや Claude Code を隣ペインで起動できます
- **置き場所**: リポジトリの隣の `<repo>.worktrees/<ブランチ名>/` に作られ、エクスプローラ上ではタイルにブランチ名バッジが付きます
- **一覧と掃除**: Git ビューの Worktrees セクションで各 worktree の状態（dirty / ↑↓ / merged）を確認。マージ済みの worktree は「掃除」1 クリックでフォルダとブランチをまとめて削除。Finder などで直接消した残骸は「管理情報を整理（prune）」で片付きます
- 未コミットの変更がある worktree の削除は警告付き（「変更ごと削除」を明示選択しない限り消えません）

> `node_modules` や `build/` などの git 管理外ファイルは worktree にコピーされません（git の仕様）。切った直後のセットアップは、開いたセッション内で実行してください。
...
ペイン内で実行中の CLI ツールが出す **通知エスケープシーケンス（OSC 9 / OSC 777）** を、Roola が OS 通知に中継します（ADR-0066）。iTerm2 / Ghostty / kitty などのモダンターミナルと同じ仕組みで、**フック登録や設定は一切不要** — インストール直後から動きます（初回の通知時に OS の通知許可を求めるダイアログだけ出ます）。

![Task completion notification](./docs/images/notification.png)

Claude Code の場合、通知が出るタイミングは次の 2 つです:

| イベント | タイミング |
|---|---|
| 許可待ち（"Claude needs your permission"） | 発生した瞬間 |
| 入力待ち（"Claude is waiting for your input"） | ターン完了から約 60 秒放置で発火（claude の標準設定 `messageIdleNotifThresholdMs` で短縮可能） |

- **通知クリックで該当ペインへフォーカスが戻ります**（タブを閉じた後の通知クリックは何もしません）
- **見ているペインからは通知しません** — フォーカス中のペイン（とフォーカス状態を受け取った claude 自身）は通知を抑制します
- claude 以外のツールも `printf '\e]9;本文\a'`（OSC 9）や `printf '\e]777;notify;タイトル;本文\a'`（OSC 777）で同じ通知が出せます
- 同一ペインからの連続通知はレート制限されます（通知シーケンスを含むファイルの `cat` などによる通知洪水の対策）

> 旧方式（Stop フック + ローカル HTTP 受信口・ADR-0057）は **撤去済み** です。旧バージョンでフックを登録（自動インストール）していた場合、残ったフックは無害（静かに失敗するだけ）ですが、`~/.claude/settings.json` の `hooks.Stop` から該当エントリを手動で削除することをお勧めします。
...
| タスク通知が出ない | ①該当ペインがフォーカス中でないか（見ている画面からは通知しない仕様。別ペインかアプリへ切り替えて確認） ②OS の通知設定で Roola が許可されているか ③claude の完了通知はターン完了の瞬間ではなく約 60 秒放置後に出る仕様（上記「タスク通知」参照） ④`printf '\e]9;test\a'` をペインで実行して切り分け（これで出れば受信側は正常） |
| Windows で worktree の作成に失敗する（パス長エラー） | リポジトリが深い階層にあると `<repo>.worktrees/<ブランチ名>/` が Windows のパス長上限（260 文字）を超えることがあります。`git config --global core.longpaths true` を設定するか、浅い階層のリポジトリで使用してください |
| 永続化されたエントリ・設定をリセットしたい | `make reset`（prod / dev の Application Support 配下を両方削除） |

## 既知の制約

- Linux は非対応
- App Sandbox は **無効化** されています（macOS。PTY 起動と任意ディレクトリへのアクセスのため）
- **Windows 固有**: ターミナルは xterm.js（WebView2）ベース
